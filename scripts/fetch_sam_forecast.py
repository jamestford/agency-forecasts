...
def main() -> int:
    ...
    print(f"[fetch] payload: {len(new_body):,} bytes, sha={new_sha[:12]}", flush=True)

    current_dir, archive_dir, meta_path = common.agency_paths(AGENCY)
    current_dir.mkdir(parents=True, exist_ok=True)

    prior_file = common.find_prior_file(archive_dir, ".json", today)
    prior_rows: dict[str, dict] = {}
    if prior_file is not None:
        try:
            prior_payload = json.loads(prior_file.read_text())
            for o in prior_payload.get("opportunities", []):
                if o.get("noticeId"):
                    prior_rows[o["noticeId"]] = o
        except Exception as e:
            print(f"[fetch] (warn) failed to parse prior snapshot {prior_file}: {e}", flush=True)
    ...
    changed = bool(d["added"] or d["removed"] or d["changed"]) or prior_file is None
    ...
    if changed:
        ...
        if prior_file is not None:
            md = diff_lib.render_markdown(
                d, old_date=prior_file.parent.name, new_date=today
            )
        ...
        print("  -> no row-level changes vs prior snapshot", flush=True)
